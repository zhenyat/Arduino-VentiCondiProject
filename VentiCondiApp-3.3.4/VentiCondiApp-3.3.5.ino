 ...
 *    31.08.2021  3.3.4   Tested & finalized
 *    12.01.2022  3.3.5   Condi Power max = 50%
 *************************************************************************/
#include <zt_venti_condi.h>

/*  App params  */
const bool DEBUG = true;
const char* APP_VERSION = "3.3.5 (12.01.2022)";
...
const bool SET_CONDI_UPPER_LEVEL = true;
const int CONDI_UPPER_LEVEL = 4; // 50% Condi power
...
//  Serial.println("+++++ ZT! After Condi ON...\n");
  ...
  if (targetCondiLevel > 0) {
    if (SET_CONDI_UPPER_LEVEL && targetCondiLevel > 4) {
      serial_printf(Serial,"\n+++++ Setup: Force Condi power down from %d\%% to %d\%%", CONDI_POWER[targetCondiLevel], CONDI_POWER[CONDI_UPPER_LEVEL]);
      targetCondiLevel = CONDI_UPPER_LEVEL;
    }
...
//    serial_printf(Serial, "\n!!!!! 2-After getCondiLevel: targetCondiLevel = %d, actualCondiLevel = %d\n", targetCondiLevel, actualCondiLevel);
    ...
    if (trend == "Standby") {
      ...
      if (trend == "Up") {
        if (SET_CONDI_UPPER_LEVEL && targetCondiLevel > 4) {
          serial_printf(Serial,"\n+++++    Up: Force Condi power down from %d\%% to %d\%%", CONDI_POWER[targetCondiLevel], CONDI_POWER[CONDI_UPPER_LEVEL]);
          targetCondiLevel = CONDI_UPPER_LEVEL;
        }
