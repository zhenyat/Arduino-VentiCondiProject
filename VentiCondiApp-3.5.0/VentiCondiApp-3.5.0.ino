 ...
 *            
 *  Input Analog signal:
 *    - Voltage range 0...5 V converted (divided) by the potentiometer (10kΩ) 
 *      from Venti Voltage range 0...10 V 
 *    - PWM range 0...1023
 *    - 9 levels of Venti Voltage
 *  
 *  Output Analog signal: 
 *    - Condi Voltage range 0...5 V
 *    -       PWM range     0...255
 *    - 8 levels of Conditioner Power (Performance)
 *   
 *        Input: Venti                              Output: Condi 
 *   =============================  ------------------------------------------------------    
 *   Level  Original    Divided     Level  Control_Voltage   Power Coded_Voltage PWM_Units 
 *          Voltage  Voltage Units         Min  Mean   Max           Up  Down    Up  Down
 *     0      0.0       0.0     0     0   0.00        1.25    0   (1.20) 1.00   (61)   51
 *     1      0.1      0.05    10     1         1.75         10%   1.80  1.40    92    71  
 *     2      2.5      1.25   255     2         2.25         20%   2.50  2.00   128   102   
 *     3      4.5      2.25   460     3         2.75         30%   3.00  2.40   153   122
 *     4      6.0      3.00   613     4         3.25         50%   3.50  2.90   179   147     
 *     5      7.5      3.75   767     5         3.75         70%   4.00  3.50   204   178
 *     6      8.5      4.25   69      6         4.25         80%   4.40  4.20   224   214
 *     7      9.5      4.75   971     7   4.75        5.00  100%   4.80 (5.00)  245  (255)
 *     8    (10.0)    (5.00)(1023)
 *   
 *  Remarks to the Table:
 *  1) Coded_Voltage / PWM_Units to control the Condi are defined by measurements
 *  2) Values in parantheses () are not used in the Code below - just to fill arrays completely
 *
 *    Libraries used:
 *        ~/Arduino/libraries/
 *                       |
 ...
 *    11.01.2022  3.5.0   Revised according to working experience (3.4.0 skipped as not correct one)
 *****************************************************************************/
#include <zt_venti_condi.h>

/*  App params  */
const bool DEBUG = true;
const char* APP_VERSION = "3.5.0 11.01.2022)";
char printIntValue[4];       // to print integer values with '%4d' format

/*  Arduino params  */
const unsigned int BAUD_RATE = 9600;
const unsigned int VENTI_PIN = A0; // Arduino analog input  pin (Venti System)
const unsigned int CONDI_PIN = 3;  // Arduino analog output pin (PWM for Condi)

/*  Venti params  */
const float VENTI_ORIGINAL_VOLTAGE_MAX = 10.0;
const unsigned int VENTI_UNIT_MAX = 1023;
const unsigned int VENTI_BOTTOM_PWM = 10;   // 0.10- V means that Venti requests   0% Power
//const unsigned int VENTI_TOP_PWM  = 1002; // 4.90+ V means that Venti requests 100% Power 
const unsigned int VENTI_CHECK_BOTTOM_ATTEMPTS = 5;
...
const unsigned int TIME_CHECKING_VENTI_STATUS   =  15;
const unsigned int DELAY_BEFORE_SMOOTH_INCREASE = 180;
const unsigned int SMOOTH_INCREASE_DELAY        =  20;
...
void setup() 
{
  ...
  if (targetCondiLevel > 0) {
    for (int level = 1; level <= targetCondiLevel; level++) {
      ...
      if (level == 1) {
        if (DEBUG) serial_printf(Serial,"\n----- Let's stay at the Condi power %d\%% %d seconds...", CONDI_POWER[level], DELAY_BEFORE_SMOOTH_INCREASE);
        sleep(DELAY_BEFORE_SMOOTH_INCREASE);
      } else {
        if (DEBUG) serial_printf(Serial,"\n----- Smooth Condi power increasing: %d\%%", CONDI_POWER[level]);
        sleep(SMOOTH_INCREASE_DELAY);
      }
    }
  }
